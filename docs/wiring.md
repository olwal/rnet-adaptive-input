...
Divider design for a 3.3 V ADC, the Teensy 3.0 pad map, expected readings, and known issues.
...
`card5a` / `card5b`). Not redistributed here; download them separately if you
want them locally.
...
Analog inputs are **not** separate pads. `A0` to `A9` are digital pads 14 to 23
...
Divider bottom legs return to the shared digital GND, which is correct. AGND is
tied to GND internally on Teensy 3.x, so the choice is a noise preference rather
than a wiring requirement. Move them to AGND only if the readings turn out noisy.

Note that the `3.3V` pad sits two positions left of pad 23, so take care not to
bridge a divider tap into it.
...
- **Vin accepts 3.7–5.5 V only.** Never connect the 12 V joystick supply to it.
  Power the Teensy from USB, which is needed for the serial link in any case.
  The two supplies stay separate and share only ground.
- **`3.3V` is an output, 100 mA max.** It supplies the dividers' reference, not the joystick.
...
- **Vref (A2)**: ~700 counts (2.26 V at ADC), and should be steady.
...
Teensy 3.0 supports `analogReadResolution(12)` for 4× the resolution (Vref ≈ 2800, swing ≈ ±572). It is one line in `setup()`, commented out in the sketch. The voltages don't change, only the scale.

## Field notes

- **High ADC source impedance.** The 100k/68k dividers
...
- The joystick is **fully functional and unmodified** as of last session. Do not open the housing unless further investigation requires it.
...
- The 1.8 kΩ series resistors inside the joystick on the X/Y outputs provide modest short-circuit protection on those pins, but Vref (470 Ω series in Bob's schematic, may be similar inside the real joystick) is less protected, so do not short pin 3 to anything.
