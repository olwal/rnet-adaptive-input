...
|---|---|---|
| **Tetris** | Move = stick L/R; soft/hard drop = down; **rotate = flick** | Rotate is the only discrete verb — bind it to flick-up (or a quick rim gesture). The rest is continuous. |
...
**Implementation note for Tier B:** the only real work is the button-synthesis layer. In priority order: **flick-and-release** (best — real onset/offset) → **magnitude threshold** (hard push = act, soft = move) → **dwell-in-direction** (hold a heading N ms) → **rim/rotation gesture** (reload, cycle, confirm) → **center-hold as an explicit state** (cancel / safe / commit-release).

---

## Part 2 — Proposed original dynamics

These lean into the stick instead of adapting to it. The richest vein: **the input transforms the space and an automatic process resolves the result** — the echochrome / Katamari / Fez family, where there's no "do the thing" button because *the thing does itself*.
...
- Pulls toward slow, tense pacing — good for accessibility.
...
**Pros**
- Genuinely button-free by design, not by adaptation.
...
- Emergent difficulty from your own accumulated marks — high replay value from simple rules.
...
- **Fastest to a playable prototype:** Snake variant or Tetris-with-flick-rotate (Part 1) — validates the flick-synthesis layer with almost no new design.
- **Most original but low-risk on input:** Gravity-flip (2.2) — sidesteps the magnitude-legibility problem entirely.
